...
ispc_flags = -g -O3 --target avx2-i64x4 --cpu=core-avx2 $(particles)
viz: viz.o $(OBJECTS)
...
.PHONY: clean all
EOF
srun make clean && make && perf stat -r 2 ./tiny_md
...
vGFLOPS=$(tail -n 4 statics.res | grep GFLOPS| cut -d : -f 2 | xargs)
vinsn=$(tail slurm-$SLURM_JOB_ID.out | grep insn | cut -d '#' -f 2 | cut -d i -f 1| xargs)
echo "gcc-5 -O3 -march=native",$vtime,$vtimeerr,$vGFLOPS,$vinsn >> results.plot
#####################################################################################################
cat > Makefile <<'EOF'
CC      =  icc #clang
